...
async function handleItemClick(e) {
    ...
    loadSingers(data, document.querySelector(".listSong-infor__singers"));
...
function loadListContent(data) {
    ...
    <p class="listSong-content-list-item__album" data-id="${data.album.encodeId}">
        ${data.title}
    </p>
...
`;
    ...
    let singers = item.querySelector(
        ".listSong-content-list-item-infor-song__singer"
    );
    data.album.artists.forEach((item) => {
        const template2 = ``;
    });
    return item;
}
